...
K = length(log_prior);
n_examples = size(test_data, 2);

log_prob = zeros(K, n_examples);

for k = 1 : K
    mean_mat = repmat(class_mean(:,k), [1,n_examples]);
    var_mat = repmat(class_var(:,k), [1,n_examples]);
    log_prob(k,:) = sum(-0.5 * (test_data - mean_mat).^2 ./ var_mat - 0.5 * log(var_mat), 1) + log_prior(k);
end

[~, prediction] = max(log_prob, [], 1);
